...
class PlayerQuit : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if(args?.get(0) !== null){
            ...
            pluginInstance.config.isNotificationsPlayerQuitEnabled = value
            pluginInstance.config.save()

            sender.sendMessage("Player quit notification ${if(value) "enabled" else "disabled"}")
            return true
        }
